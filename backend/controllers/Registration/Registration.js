...
const registerFarmer = async (req, res) => {
  ...
  try {
    ...
    if (existUser) {
      ...
      const b64_aadhar = Buffer.from(req.files["aadharImage"][0].buffer).toString("base64");
      let aadhar_URI = "data:" + req.files["aadharImage"][0].mimetype + ";base64," + b64_aadhar;
      const cldRes_aadhar = await handleUpload(aadhar_URI, "PDS_System");

      const b64_user = Buffer.from(req.files["userImage"][0].buffer).toString("base64");
      let user_URI = "data:" + req.files["userImage"][0].mimetype + ";base64," + b64_user;
